...
%%%% set filter name
    case 'uniform'
        %%%%% 1. all 256 neurons
        versionName = 'subset_32_256_random_1000'; % version of neuron filters
        dataFolder   = '/Users/liushizhao/projectData_local/probinf_data/syntheticData_interleaved/synthData_use_interleaved/real_interleaved/batch_3_timebin';
        %%%% on linux
        if ~exist(dataFolder)
            dataFolder = '/home/shizhao/Documents/projectData/probinf_data/syntheticData_interleaved/synthData_use_interleaved/real_interleaved/batch_3_timebin';
        end
    case 'subsample_test'
        %%%%% 2. test subsampling
        filter_folder = '../../results/filtered_neuron_synthetic/subsample_random';
        filer_list = dir(fullfile(filter_folder,'*.mat'));
        n = 6;
        versionName = filer_list(n).name;
        load(sprintf('../../results/filtered_neuron_synthetic/filtered_neurons_%s.mat',versionName));
        dataFolder   = '/Users/liushizhao/projectData_local/probinf_data/syntheticData_interleaved/synthData_use_interleaved/real_interleaved/batch_1';
        %%%% on linux
        if ~exist(dataFolder)
            dataFolder = '/home/shizhao/Documents/projectData/probinf_data/syntheticData_interleaved/synthData_use_interleaved/real_interleaved/batch_1';
        end
    case 'match_empirical'
        %%%%% 3. subsample based on empirical data
        b_PF_list = [0.8,0];
        ...
        % b_PF = 0.8;
        %%%% on macboox
        dataFolder   = '/Users/liushizhao/projectData_local/probinf_data/syntheticData_interleaved/synthData_use_interleaved/real_interleaved/batch_2';
        %%%% on linux
        if ~exist(dataFolder)
...
switch versionOption
    case 'uniform'
        %%%% specific sessions
        data_list(1).name = 'synthData_use_interleaved_bPF_0_00_cardinal_delta_0_08_prior_1_00_oblique_delta_0_08_prior_1_00_timebin.mat';
    case 'subsample_test'
        %%%% specific sessions
...
if doRun_cross
    for n = 1:nSession
        ...
        if doMultipleTimebin
            saveName   = fullfile(saveFolder_session,sprintf('%s_timebin.mat',sessionStr));
        else
