function run_directFisherInfoEstimate_versionControl(filter_name,runOptions,combineOptions)
% clear all
% clc
% close all
...
session_list_gr_all    = [session_list_gr.oblique_first_training;session_list_gr.oblique_normalSize_list;session_list_gr.cardinal;session_list_gr.interleaved];
...
% 
%         filelist = dir(fullfile(taskDataPath,sprintf('%s_*_%s_*extractedData.mat',sessionStr,'bpGratingFC')));
%         if numel(filelist) == 0
%             error(sprintf('session %s task data can not be found',sessionStr));
%         end
% 
%         filelist = dir(fullfile(oriDataPath,sprintf('%s_*_%s_*extractedTuningData.mat',sessionStr,'rfMapping_bpGrating')));
%         if numel(filelist) == 0
%             warning(sprintf('session %s orientation data can not be found',sessionStr));
%         end
% 
% 
%         filelist = dir(fullfile(unitPropertiesPath,sprintf('%s_unitProperties.mat',sessionStr)));
%         if numel(filelist) == 0
%             error(sprintf('session %s unit property can not be found',sessionStr));
%         end
%     end
    ...
    for i_session = 1:nSession
        ...
        if ~run_decode & ~run_decode_cross
            % do not run neither, skip
            continue
        end
        
        if ~run_decode & run_decode_cross & ~ismember(sessionStr, interleaved_session_all) 
        ...
        if run_decode_cross & ~isempty(dat_fisher_cross)
...
%% organize cross-decoding results of all individual sessions into one big struct
if doOrganizeCross
...
%% Combine fisher information estimate across difference coherence level
if doCombineAcrossCoherence
   dataName = fullfile(saveFolder, sprintf('fisherInfo_direct_all_sessions_%s',filter_name));
   load(dataName);
   saveName = fullfile(saveFolder,sprintf('CoherenceCombine_fisherInfo_all_sessions_%s', filter_name));
