function dat_cross_fisher = run_fisher_cross_estimate_one_session_timebin(dat_cross_fisher,data_run,info_run)
...
spikeCount_whole            = data_run.spikeCount_whole;
spikeCount_timebin          = data_run.spikeCount_bin;  
...
for i = 1:numel(signal_abs_list) % run analysis for each coherence level
    ...
    X_zero_cardinal   = spikeCount_timebin(idx_cardinal_zero,:);
    X_zero_oblique    = spikeCount_timebin(idx_oblique_zero,:); 
    ...
    X_cardinal_pos_all           = spikeCount_whole(idx_cardinal_pos,:); 
    X_cardinal_neg_all           = spikeCount_whole(idx_cardinal_neg,:);
    X_cardinal_all               = [X_cardinal_pos_all;X_cardinal_neg_all];
    Y_cardinal_all               = [ones(T1_cardinal,1); -1 * ones(T2_cardinal,1)];

    X_oblique_pos_all           = spikeCount_whole(idx_oblique_pos,:); 
    X_oblique_neg_all           = spikeCount_whole(idx_oblique_neg,:);
